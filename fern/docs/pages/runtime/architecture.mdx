...
The broker starts after that decision. It uses the right user/Codex-profile auth home, starts or reuses a `codex app-server` child, maps the broker thread to the underlying Codex thread, prevents another turn from racing on the same support conversation, mounts the bundle, persists normalized events, and streams those events back to the app.
...
| `src/codex_broker/auth.py` | User hashing, Codex auth profile normalization, `CODEX_HOME` creation, Codex login/status/logout. |
...
The pool key includes user hash, Codex auth profile, auth fingerprint, `configProfile`, Codex command/version, credential-store mode, client identity fields, process-level Codex config, mounted MCP declarations, and hashes of resolved MCP `env:VAR` values.
