...
| `auth/owners/<owner-hash>/profiles/<profile>/codex-home` | Per-user/per-Codex-auth-profile Codex home. The directory still uses `owners` because the API field is `ownerId`. |
...
| `profile` | User's Codex auth profile. Defaults to `default`; returned value is canonicalized. |
...
| `profile` | Overrides the thread's Codex auth profile for this turn. |
