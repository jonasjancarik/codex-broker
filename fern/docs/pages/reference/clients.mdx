...
Host services can call the native broker API directly over HTTP, use the
lightweight native clients in this repository, or point an OpenAI SDK at the
compatible API.

## OpenAI SDKs

For compatible text or image Responses and Chat Completions, use the official OpenAI SDK
with a broker-issued compatibility key:
...
The compatibility surface does not expose native broker thread, interaction,
auth-management, or audit routes. See
[OpenAI compatibility](../integrations/openai-compatibility.mdx) before
reusing an integration that sends tools, unsupported non-text content, sampling
controls, or `store: false`. Compatible image input must use base64 data URLs.
Chat `max_tokens` and `max_completion_tokens` are accepted and ignored, while
Responses `max_output_tokens` is rejected.
...
print(final_turn["usage"])
```

The Python client returns plain dictionaries and raises `CodexBrokerClientError`
for HTTP errors. `final_turn["usage"]` contains exact turn and cumulative thread
token counts when Codex reported them; otherwise it is `None`.
...
```ts
import { CodexBrokerClient, type TurnUsage } from "./codex-broker-client";
...
## Native event streaming in Node
...
for await (const event of streamBrokerEvents(
  ...
  if (event["type"] === "turn.usage.updated") {
    const payload = event["payload"] as { usage: TurnUsage | null };
