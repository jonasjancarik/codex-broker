...
This matrix is pinned to the Codex CLI app-server protocol generated from `codex-cli 0.142.3`:

```bash
codex app-server generate-ts --out /private/tmp/codex-app-server-ts-0.142.3
codex app-server generate-json-schema --out /private/tmp/codex-app-server-json-schema-0.142.3
```
...
| Capability | 0.142.3 app-server surface | Broker support | Host-facing surface |
| --- | --- | --- | --- |
...
| Tool/file/permission approvals | `item/commandExecution/requestApproval`, `item/fileChange/requestApproval`, `item/permissions/requestApproval`, legacy `applyPatchApproval`, `execCommandApproval` | Implemented with safe default responses. The broker declines command/file requests, denies legacy requests, and grants no extra permissions for permission-profile requests. | `tool.requested`, `approval.requested`, `approval.resolved`, owner audit entries |
| User input prompts | `item/tool/requestUserInput` | Implemented with safe default empty answers; host-mediated answer API not yet implemented | `user_input.requested`, `user_input.resolved` events |
| MCP elicitations | `mcpServer/elicitation/request`; initialize capability `mcpServerOpenaiFormElicitation` | Implemented with safe default decline; host-mediated elicitation API not yet implemented | `mcp.elicitation.requested`, `mcp.elicitation.resolved` events |

## API Direction

The broker currently treats app-server mode-like behavior as observable event state unless there is already a stable product-facing API. This keeps host integrations compatible with the generated 0.142.3 protocol without inventing app-server methods that are not exposed.
...
- Approvals, user input, and MCP elicitations: add pending-request storage plus host response endpoints before changing safe defaults. Until then, automatic decline/no-answer behavior is deliberate because the broker must never hang an app-server child waiting for a host UI that may not exist.
